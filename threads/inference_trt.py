...
# from pycuda import autoinit # для стереостражника отключена
# from modules.logger import CustomLogger # запись в journalctl
...
class TrtYOLO:
    ...
    def __init__(self, engine_path: str, input_shape: list, conf=0.25, iou=0.45):
        ...
        # self.logging = CustomLogger(__name__)
        ...
        if FLAG_DEBUG:
            print("engine load")
        # else: self.logging.warning("engine load")
        self.max_batch_size = 1
        self.inputs, self.outputs, self.bindings, self.stream = self.allocate_buffers()
        self.context = self.engine.create_execution_context()
        if FLAG_DEBUG:
            print("done!")
        # else: self.logging.warning("done!")
    ...
    def __call__(self, x:np.ndarray) -> np.ndarray:
        ...
        boxes = self.outputs[0].host.reshape(6, -1).T  # TODO: change to n_classes + 4
        boxes = boxes[np.logical_or(boxes[:, 4] > self.conf,boxes[:, 5] > self.conf ), :]  # filter boxes by confidence
        # print(boxes)
    ...
    def draw_bboxes(self, frame: np.ndarray, boxes: np.ndarray) -> np.ndarray:
            ...
            for box in boxes:
                #DEBUG
                # print(f"class: {int(box[5])} , conf {int(box[4])}")
                frame = cv2.rectangle(frame, (int(box[0]), int(box[1])),
                                    (int(box[2]), int(box[3])), (0, 0, 255), 2)
                frame = cv2.putText(frame, f"Class: {int(box[5])} conf: {box[4]}", (int(box[0]), int(box[1])),
...
____________________EXAMPLE_______________________
...
#     # cap = cv2.VideoCapture("rtsp://admin:UniVision_74_ceil@192.168.0.74:554/media/video2")

#     cap = cv2.VideoCapture("/home/techtrans/dev/videos_svo/HD2K_SN34222655_4.svo")

#     # model = TrtYOLO("/home/techtrans/dev/weights/vids.engine",(320,320))
    

#     model = TrtYOLO("/home/techtrans/dev/weights/ultra_new_320.engine",(320,320))
...
#             print(boxes)
#             try:
#                 img = model.draw_bboxes(img, boxes)
#             except: pass
#             cv2.imshow("img",img)
#             if cv2.waitKey(25) & 0xFF == ord('q'):    
#                     break
#             print(round(1/(time.time()-start_),2))
#             start_ = time.time()
#     cap.release()
